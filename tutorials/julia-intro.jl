...
#> title = "Julia简介"
#> date = "2023-12-09"
#> description = "介绍Julia语言的特点，安装和Pluto Notebook工作环境"
#> 
#>     [[frontmatter.author]]
#>     name = "陈伟凯"
...
# ╔═╡ eb6fe3ce-7451-46f8-92dd-9d0701b65a29
# ╠═╡ show_logs = false
begin
...
# ╔═╡ ec1a52a2-cf60-4b94-a8ee-c2bd4128e4ac
begin
	using PlutoUI
	TableOfContents(title="📚目录", depth = 4)
end

# ╔═╡ 96c5a756-9488-4915-940c-3cdddd34abac
begin
	...
	Show(MIME"text/html"(), String(regtable(rr1,rr2,rr3; renderSettings = htmlOutput(), print_result = false)))
end

# ╔═╡ 9fa22e00-91b8-11ee-103a-97a9c95f8f73
md"""
# Julia 简介
...
本节主要介绍Julia编程语言的特点、安装和工作环境。
"""

# ╔═╡ f52c4cae-148b-4f28-9141-568c85d805d1
md"""
## 一、Julia 语言简介
...
Julia编程语言（[https://julialang.org/](https://julialang.org/)） 发布于2012年， 由MIT的几位作者（Jeff Bezanson, Stefan Karpinski, Viral Shah, Alan Edelman）和全世界的参与者共同制作的。
	
Julia是一种专为高性能数值计算和科学计算而设计的编程语言。Julia与R、Python一样简单， 但是它先进的设计使得Julia程序的效率基本达到和C、Fortran等强类型语言同样的高效率，尤其适用于数值计算和数据分析，已经获得广泛的关注。 Julia语言的特点包括：

- **高性能**：Julia使用即时编译技术，可以在运行时生成高效的本地机器代码。这使得Julia在处理大量数据和复杂数学运算时具有很高的性能。由于Julia用LLVM虚拟机执行， 这使得其程序在初次运行时好像响应很慢， 这其实是在进行即时编译， 用编译的等待换取运行时的高效率， 对于不能向量化的程序， Julia可以比R、Python这些动态语言快一个数量级。
- **动态类型**：Julia支持动态类型，这意味着你可以在编写代码的过程中随时更改变量的类型。这大大简化了编程过程，使得Julia更加易用。
- **丰富的库和包**：Julia拥有大量的高质量库和包，涵盖了从线性代数、优化、概率统计到机器学习等各种领域。这些库和包可以帮助你快速实现各种经济模型和算法。

Julia 最吸引我们的地方在于以下两个方面的特性：

1. Julia 解决了所谓的“两语言问题”。 
!!! info "两语言问题"
	这是科学计算中的典型问题，通常研究人员想要设计一种算法或方案来解决手头的问题或分析。 一般地，解决方案的原型代码都采用容易编程的语言（像 Python 或 R）。 如果原型能够正常工作，那么研究人员就会使用不易编写原型但快速的语言（C++ 或 FORTRAN）重新实现。 因此，开发解决方案的过程涉及了两种语言。一种语言易于编写原型代码并不适合方案实现 (通常由于缓慢的速度)。 而另一种语言并不易于编写原型代码，但由于非常快，所以适合方案实现。Julia 能够避免此类情形，因为 开发原型（易编程）和方案实现（速度快）将采用相同的语言。

2. Julia 实现了“代码和数学关系的一对一”。 Julia 允许使用 Unicode 字符作为变量或参数。 这意味着不再使用 `sigma` 或 `sigma_i`，而是像数学记号那样使用 $\sigma$ 或 $\sigma_ᵢ$ 。 当查看算法代码或数学方程时，你会看到几乎相同的符号和术语。这一特性大大提高了代码的可读性，使得程序不再只是人告诉机器的指令，还可以用于人与人的沟通。

"""

# ╔═╡ 6dedd54a-bc27-441d-a4b1-9c7ca4b858dd
md"""
关于Julia的这些特性， Alan Edelman 在下面的演讲中做了说明。（[B站视频](https://www.bilibili.com/video/BV1z54y1C7KB/?share_source=copy_web&vd_source=5b707d20a4b38f833d304d4b52a572d3))
"""

# ╔═╡ efc28f7a-f095-4700-aa3c-2b720c80c148
html"""
<iframe width="560" height="315" src="https://www.youtube.com/embed/qGW0GT1rCvs?si=TFi97dcP2x7BWNUn" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
"""

# ╔═╡ 56a40e65-867f-4c09-ac50-bd7161d07f23
md"""
需要说明的是，Julia和R、Python、Matlab等一样都只是工具，有优点和缺点。
"""

# ╔═╡ cd905216-0346-4e81-8d27-82cbf0a47672
md"""
### 2. 示例

我们接下来通过几个例说明Julia可以完成的工作。

#### （1） 计量模型
"""

# ╔═╡ 04a6c58c-3aeb-4ccd-a376-7288f2e2cde5
md"""
#### （2） 解微分方程
...
# ╔═╡ bc9d20bc-9b2d-44ba-86ae-889795565662
@bind timespan PlutoUI.Slider(1:200,default=100,show_value=true)

# ╔═╡ 89c225bf-560d-4582-9906-df09c4805121
begin
...
# ╔═╡ 371a51a9-f1cd-4373-b314-1e2b5b04bbd0
Plots.plot(sol,idxs=(1,2,3))

# ╔═╡ 9d219c1e-11c0-462d-a7ff-cea767bc8616
md"""
#### （3） Agent-based Models
"""

# ╔═╡ 57a40ba2-3572-4ed5-89c8-ebce07b3aca9
# ╠═╡ show_logs = false
md"""
abmvideo(
...
# ╔═╡ 0ed3f982-b0cd-42fb-ac31-7dce446f4908
Resource("https://juliadynamics.github.io/Agents.jl/stable/examples/schelling.mp4")

# ╔═╡ d4fce34d-90da-4a2b-b880-e6cf62334fd9
md"""
### 3. 学习资源

- [官网资料： Get started with Julia](https://julialang.org/learning/)
- [Interactive Computational Thinking — MIT](https://computationalthinking.mit.edu/Fall23/)
- [Quantitative Economics with Julia](https://julia.quantecon.org/intro.html)
- [北大李东风：Julia语言入门](https://www.math.pku.edu.cn/teachers/lidf/docs/Julia/html/_book/)
...
# ╔═╡ b8ff7b72-ed53-4526-85a3-95a2ec6ae447
md"""
## 二、Julia 安装及工作环境

### 1、下载安装

访问Julia官方网站[(https://julialang.org/downloads/)](https://julialang.org/downloads/)或[国内镜像网站](https://mirrors.tuna.tsinghua.edu.cn/help/julia-releases/)根据自己的操作系统下载相应的安装包进行安装。 安装后运用Julia，即可进入命令行的界面（Julia REPL）。

![Julia REPL](https://user-images.githubusercontent.com/6933510/91439734-c573c780-e86d-11ea-8169-0c97a7013e8d.png)
"""

# ╔═╡ f1665cfe-73b7-41f0-8261-a70e75c2863a
md"""
### 2、使用国内镜像

为了加速Julia软件包的下载，在国内可能需要使用镜像，一般方法可以在[这里](https://help.mirrors.cernet.edu.cn/julia/)查看。
...
- 再使用 JuliaCN 社区维护的中文本地化工具包`JuliaZH`的方法，永久使用国内镜像，顺便介绍Julia中软件包的安装和管理。

    - 安装 `JuliaZH`包：
      - 在上述命令以后，运行Julia
      - 进入Julia RELP界面后：输入 `]`,进入软件包管理模式
      - 输入 `add JuliaZH` 安装
    - 使用 `JuliaZH`包指定镜像
```julia
	using JuliaZH #使用JuliaZH包
	JuliaZH.generate_startup("TUNA") #指定镜像; "BFSU","USTC"
	versioninfo() #查看信息
```
"""

# ╔═╡ 31699dde-0a04-43ef-988c-960097f8a05e
md"""
### 3、使用Pluto Notebook

#### （1） 简介
[Pluto.jl](https://plutojl.org/) 是 Julia 编程语言中的一个交互式计算工具，它是一个基于 Jupyter 笔记本的替代品，专门设计用于提高 Julia 语言的交互性和可视化数据分析的体验。Pluto.jl 旨在为用户提供更快速、更灵活、更动态的工作环境，使其能够更轻松地探索、理解和分享数据分析和科学计算的结果。

以下是 Pluto.jl 的一些主要特点和优势：

1. **实时反馈：** Pluto.jl 提供实时的代码执行和输出反馈，使用户能够立即看到对代码更改的影响。这有助于加速调试和开发过程，使用户能够更迅速地迭代和改进代码。

2. **自动化依赖解析：** Pluto.jl 能够自动检测代码中的依赖关系，并在执行代码块时自动更新。这减少了手动管理依赖的复杂性，使用户能够更专注于问题的解决而不是环境配置。

3. **交互式可视化：** Pluto.jl 内置了丰富的可视化工具，用户可以通过简单的命令轻松创建图表、图形和其他可视化元素。这使得数据分析和科学可视化变得更加直观和易于实现。

4. **导出和分享：** Pluto.jl 支持将笔记本导出为多种格式，包括 Jupyter 笔记本、HTML 文件和 PDF 文件。这使得用户能够方便地分享他们的工作成果，并与其他人合作或展示分析结果。

5. **易于入门：** Pluto.jl 的界面设计简单直观，适用于各种技术水平的用户。即使是初学者也可以迅速上手，享受 Julia 语言的强大功能和交互性。
...
```
运行上述命令后将通过流浏览器打开Pluto Notebook界面（建议使用Firefox或Chrome浏览器），可以创建或打开Notebook。
"""

# ╔═╡ 493089c5-1d72-4ca0-82de-ea1132e34baa
Resource("https://user-images.githubusercontent.com/55111154/100131264-d3a3a500-2ea9-11eb-9bc1-c91ff6cb9d55.gif")

# ╔═╡ dee7f9d8-a9d5-496b-9caf-644315c6326b
md"""
!!! warning "练习"
	1. 试打开 **Featured Notebooks**， 运行并编辑。
	2. 下载并运行下列Pluto Notebooks：
	   - [Julia Basics](https://weikaichen.gitlab.io/blog/julia-basics.jl)
	   - [Julia for Scientific Computing](https://weikaichen.gitlab.io/blog/julia-scientific-computing.jl)
	   - [Juia for Data Science](https://weikaichen.gitlab.io/blog/julia-datascience.jl)
"""
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "c6f91fff3fec669bf1b6ce390ce17c20a9bcead0"
...
git-tree-sha1 = "f87ddc7cf2fa44f3ed616757c33f5c9d9c85ad58"
uuid = "9d5cd8c9-2029-5cab-9928-427838db53e3"
version = "1.10.0"

[[deps.FixedEffects]]
deps = ["GroupedArrays", "LinearAlgebra", "Printf", "StatsBase"]
git-tree-sha1 = "4b5de96c36ac13f37cb7c2bfbdecd39d1b5ead5f"
...
git-tree-sha1 = "9a4355c298afb938274d8559128fd22c020d242c"
...
[[deps.Mods]]
git-tree-sha1 = "426e93792c97dc5553018e02030a3ef526bfcb18"
